...
/**
 * Image shows the producer profile image
 *
 * @since 1.2
 * @author Joao_Flach
 */
class Image extends Widget
{
    ...
    /**
     * @var \humhub\modules\user\models\Producer
     */
    public $producer;
    ...
    public function run()
    {
        ...
        if ($this->showTooltip) {
            ...
            $this->imageOptions['data-original-title'] = ($this->tooltipText) ? $this->tooltipText : Html::encode($this->producer->name);
            Html::addCssClass($this->imageOptions, 'tt');
        }

        $this->imageOptions['alt'] = Yii::t('base', 'Profile picture of {displayName}', ['displayName' => Html::encode($this->producer->name)]);
        $html = Html::img($this->producer->getProfileImage()->getUrl(), $this->imageOptions);

        if ($this->link) {
            $html = Html::a($html, $this->producer->getUrl(), $this->linkOptions);
        }
